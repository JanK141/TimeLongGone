...
public class Enemy1Editor : UnityEditor.Editor
{
    ...
    private void DrawInPlay1(StateMachine serprop)
    {
        ...
        DrawSMProp("ProjectilesThrown", serprop.GetInt("ProjectilesThrown").ToString());
    }
    #endregion
    ...
    private void DrawInEdit1(StateMachine serprop)
    {
        ...
        if(projectilesSpots.Count>0)serprop.SetFloat("DistanceToProjectileSpot", projectilesSpots.Select(p => Vector3.Distance((target as Enemy1).transform.position, p)).Min());
